                ...
                <div class="row">
                    @foreach ($courses as $course)
                        ...
                        @else
                            <div class="col-lg-4">
                                <div class="card">
                                    <img class="card-img-top" src="{{ asset('assets/img/courses/course-' . $course->id . '.png') }}" alt="Card image cap">
                                    <div class="card-body">
                                        <a
                                                @if($course->id === current_user()->membership()->id)
                                                    href="{{ route('courses.show', $course->id) }}"
                                                @else
                                                href="{{ route('membership.create') }}"
                                                @endif
                                        >
                                            <p class="card-text">{{ \Illuminate\Support\Str::upper(str_replace('_', ' ', $course->name)) }}</p>
                                        </a>
                                    </div>
                                </div>
                            </div>
